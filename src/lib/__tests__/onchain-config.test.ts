...
import { ROBINHOOD_MAINNET_ID, ROBINHOOD_TESTNET_ID, isHexAddress, isSupportedChainId } from "@/config/env";
import { ERC721_INTERFACE_ID, erc1167Bytecode, isErc1167CloneOf } from "@/lib/abi";
import { UNISWAP_FEE_TIER, UNISWAP_TICK_SPACING, chainById, explorerTxUrl, uniswapMainnet } from "@/lib/chain";
...
describe("chain configuration", () => {
  ...
  it("resolves chains and Blockscout explorer links", () => {
    ...
    expect(explorerTxUrl(46630, "0xabc")).toBe("https://explorer.testnet.chain.robinhood.com/tx/0xabc");
  });
  ...
  it("pins the official Robinhood Chain Uniswap v3 deployment", () => {
    ...
    expect(uniswapMainnet.positionManager.toLowerCase()).toBe("0x73991a25c818bf1f1128deaab1492d45638de0d3");
...
describe("ERC-1167 clone verification", () => {
  ...
  it("builds the canonical 45-byte minimal proxy runtime", () => {
    ...
    expect(code).toBe("0x363d3d373d3d3d363d731234567890abcdef1234567890abcdef12345678" + "5af43d82803e903d91602b57fd5bf3");
...
describe("terms hashing", () => {
  it("hashes the 2026-09-01 terms body to the value stored on-chain", () => {
    ...
    expect(keccak256(toBytes(body))).toBe("0x473d0eeaaf7b4ec251c2bf167e865d0e5e1c9d15574572e2cd93c62367efb8a3");
  });
});
...
describe("IPFS CIDv1 (raw, sha2-256, base32)", () => {
  ...
  it("builds the same CID from a precomputed digest", async () => {
    const digest = new Uint8Array(await crypto.subtle.digest("SHA-256", new TextEncoder().encode("hello world")));
    expect(cidV1RawFromDigest(digest)).toBe("bafkreifzjut3te2nhyekklss27nh3k72ysco7y32koao5eei66wof36n5e");
