  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "YARD SALE Launchpad" },
      {
        ...
        content: "Turn a listing into a verifiable Item Passport on Robinhood Chain.",
      },
...
function Launchpad() {
  ...
  const selected: LaunchpadItem | null =
  ...
  const missing = useMemo(() => {
    ...
    if (state.data && linkedWallets.length === 0)
      list.push("a wallet linked to this account in Settings");
    if (account && !accountLinked)
      list.push(
        `the connected wallet ${shortAddress(account)} to be linked to this account (or switch to a linked one)`,
      );
    return list;
  ...
  async function ensureWallet(): Promise<string> {
    ...
    if (!linkedWallets.includes(acct)) {
      throw new Error(
        `Wallet ${shortAddress(acct)} is not linked to this account. Link it in Settings or switch wallets.`,
      );
    }
  ...
  async function onFreeze() {
    ...
    await run("freeze", async () => {
      if (!attestationComplete(attestation))
        throw new Error("Confirm every possession statement first.");
  ...
  async function onPrepareMint() {
    ...
    await run("prepare-mint", async () => {
      ...
      if (preview.alreadyMinted) {
        toast.message(
          `A passport (#${preview.tokenId}) already exists on-chain for this listing. Recovering it…`,
        );
  ...
  async function onMint() {
    ...
    await run("mint", async () => {
      const wallet = await ensureWallet();
      if (wallet !== mintPreview.wallet.toLowerCase())
        throw new Error("The connected wallet changed. Prepare the transaction again.");
      const hash = await sendTransaction({
        from: wallet,
        to: mintPreview.to,
        data: mintPreview.data,
        gas: mintPreview.gasEstimate
          ? `0x${((BigInt(mintPreview.gasEstimate) * 120n) / 100n).toString(16)}`
          : undefined,
      });
  ...
  async function onPrepareToken() {
    ...
    await run("prepare-token", async () => {
      ...
      const wallet = await ensureWallet();
      const preview = await planToken({
        data: { listingId: selected.listing.id, wallet, ...tokenInput() },
      });
      if (preview.alreadyCreated) {
        toast.message(
          `A companion token already exists at ${preview.tokenAddress}. Recovering it…`,
        );
  ...
  async function onCreateToken() {
    ...
    await run("token", async () => {
      const wallet = await ensureWallet();
      if (wallet !== tokenPreview.wallet.toLowerCase())
        throw new Error("The connected wallet changed. Prepare the deployment again.");
      const input = tokenInput();
      if (
        input.totalSupply !== tokenPreview.args.totalSupply ||
        input.creatorAllocation !== tokenPreview.args.creatorAllocation ||
        input.name !== tokenPreview.args.name ||
        input.symbol !== tokenPreview.args.symbol
      ) {
        throw new Error(
          "The token details changed after the preview. Prepare the deployment again.",
        );
      }
      const hash = await sendTransaction({
        from: wallet,
        ...
        gas: tokenPreview.gasEstimate
          ? `0x${((BigInt(tokenPreview.gasEstimate) * 120n) / 100n).toString(16)}`
          : undefined,
      });
      await submitToken({
        data: { listingId: selected.listing.id, txHash: hash, wallet, ...input },
      });
      setTokenPreview(null);
  ...
  function lpInput() {
    ...
    if (!/^\d+(\.\d+)?$/.test(lpTokenAmount) || !/^\d+(\.\d+)?$/.test(lpEthAmount))
      throw new Error("Amounts must be positive decimal numbers.");
  ...
  async function onPreviewLiquidity() {
    ...
    await run("lp-preview", async () => {
      const wallet = await ensureWallet();
      setLpPreview(
        await previewLp({ data: { listingId: selected.listing.id, wallet, ...lpInput() } }),
      );
    });
  }
      ...
      for (let i = 0; i < 6; i += 1) {
        ...
        if (!prep.pendingTxHash) {
          ...
          const hash = await sendTransaction({
            ...
            to: prep.to,
            data: prep.data,
            value: prep.value ?? undefined,
          });
          await recordLp({
            data: { listingId: selected.listing.id, step: prep.step, txHash: hash },
          });
  ...
  async function onResetLiquidity() {
    ...
    await run("lp-reset", async () => {
      ...
      toast.success(
        "The liquidity plan was cleared. Any approvals already on-chain remain in your wallet.",
      );
    });
  }
  ...
  async function onLockLiquidity() {
    ...
    await run("lp-lock", async () => {
      ...
      const prep = await prepareLock({
        data: { listingId: selected.listing.id, lockDays: days, permanent: lockPermanent },
      });
  ...
  const imageHashes = Array.isArray(passport?.image_hashes)
    ? (passport?.image_hashes as { sha256: string }[])
    : [];
  const attested = passport?.attestation as PossessionAttestation | null | undefined;
  const stalledAge = passport?.submitted_at
    ...
    <div className="mx-auto max-w-[1440px] px-4 py-10 sm:px-6 lg:px-10">
      <header className="max-w-3xl">
        ...
        <h1 className="mt-2 text-3xl font-extrabold tracking-tight sm:text-4xl">
          Mint an Item Passport
        </h1>
        <p className="mt-3 text-muted-foreground">
          Everything here runs against <strong>{chainName}</strong>
          {chainId ? ` (${chainId})` : ""}. Every value is shown before you sign, every transaction
          is signed by your own wallet, and nothing is recorded in YARD SALE until the receipt,
          event and resulting contract state have been verified.
        </p>
        {health && health.mainnetRequested && !health.mainnetEnabled ? (
          <p className="mt-2 text-sm text-muted-foreground">
            Mainnet (4663) is requested but{" "}
            <span className="font-mono text-xs">VITE_ENABLE_MAINNET</span> is false, so this page is
            running against the testnet. Mainnet actions stay disabled until the deployment checks
            on{" "}
      ...
      {state.isLoading ? (
        <p className="mt-6 text-sm text-muted-foreground">
          Checking the deployment and your listings…
        </p>
      ) : null}
                ...
                value={
                  walletChain === null
                    ...
                    : walletOnChain
                      ...
                      : `Chain ${walletChain} — switch needed`
                }
                mono={false}
              />
            </div>
                    ...
                    <li key={item.listing.id}>
                      ...
                      >
                        ...
                        <span className="mt-2 flex flex-wrap gap-1">
                          ...
                          {item.liquidity?.status === "confirmed" ? (
                            <StatusChip tone="grass">Liquidity live</StatusChip>
                          ) : null}
                          {item.liquidity && item.liquidity.status !== "confirmed" ? (
                            <StatusChip tone="warning">Liquidity in progress</StatusChip>
                          ) : null}
                        </span>
                      </button>
                    </li>
        ...
        <div className="space-y-5">
          {!selected ? (
            ...
            <>
              ...
              <Step
                index={1}
                title="Attest and freeze the Item Passport metadata"
                done={Boolean(passport)}
              >
                {!selected.eligibility.eligible && !passport ? (
                  <p className="text-muted-foreground">
                    {eligibilityLabel[selected.eligibility.reason]}.
                  </p>
                ) : null}
                  ...
                  <div className="space-y-1">
                    ...
                    {passport.ipfs_cid && passport.ipfs_pinned_at ? (
                        ...
                        value={
                          <>
                            ...
                            <a
                              ...
                              href={ipfsGatewayUrl(
                                `ipfs://${passport.ipfs_cid}`,
                                publicEnv.ipfsGateway,
                              )}
                              target="_blank"
                              rel="noreferrer"
                            >
                      ...
                      <Field
                        ...
                        value={
                          <>
                            <span className="font-mono">{passport.ipfs_cid}</span> — content ID
                            computed locally but not pinned (no PINATA_JWT configured). The metadata
                            is stored in YARD SALE storage and its hash is on-chain; a gateway will
                            not serve this CID until it is pinned.
                          </>
                        }
                        mono={false}
                      />
                      ...
                      <Field
                        label="IPFS"
                        value="Not pinned (no PINATA_JWT configured) — metadata is stored in YARD SALE storage and its hash is on-chain"
                        mono={false}
                      />
                    )}
                    {passport.storage_url ? (
                      <Field label="Storage copy" value={passport.storage_url} />
                    ) : null}
                    ...
                    <Field
                      label="Photo hashes"
                      value={imageHashes.length ? imageHashes.map((i) => i.sha256).join("\n") : "—"}
                    />
                    <Field
                      label="Attested"
                      value={
                        attested && attestationComplete(attested)
                          ? `All ${ATTESTATION_STATEMENTS.length} statements confirmed`
                          : "Incomplete"
                      }
                      mono={false}
                    />
                    <Field
                      label="Target"
                      value={`${passport.chain_id} · ${passport.contract_address}`}
                    />
                  </div>
                ) : null}
                      ...
                      <label key={s.key} className="flex items-start gap-3">
                        <Checkbox
                          ...
                          onCheckedChange={(value) =>
                            setAttestation((a) => ({ ...a, [s.key]: value === true }))
                          }
                          aria-label={s.text}
                        />
                ...
                {passport?.status !== "confirmed" ? (
                  <Button
                    ...
                    disabled={
                      ...
                      !chainReady ||
                      (!selected.eligibility.eligible && !passport) ||
                      !attestationComplete(attestation) ||
                ...
                {passport?.status === "submitted" ? (
                  <p className="text-muted-foreground">
                    Metadata is locked while a mint is pending.
                  </p>
                ) : null}
              </Step>
              ...
              <Step
                index={2}
                title="Review and mint the Item Passport"
                done={passport?.status === "confirmed"}
              >
                {!passport ? (
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap gap-4 pt-1">
                      <ExplorerLink href={explorerTxUrl(passport.chain_id, passport.tx_hash ?? "")}>
                        Mint transaction
                      </ExplorerLink>
                      <ExplorerLink
                        href={explorerAddressUrl(passport.chain_id, passport.contract_address)}
                      >
                        Registry contract
                      </ExplorerLink>
                    </div>
                  ...
                  <>
                    ...
                    <Field
                      label="Signer"
                      value={account ? shortAddress(account) : "Connect your wallet"}
                      mono={false}
                    />
                    ...
                    {passport.status === "submitted" ? (
                      <div className="space-y-2 rounded-lg border border-border bg-muted/30 p-3">
                        <p>
                          A mint transaction is pending:{" "}
                          <span className="font-mono text-xs">{passport.tx_hash}</span>
                        </p>
                        {passport.tx_hash ? (
                          <ExplorerLink href={explorerTxUrl(passport.chain_id, passport.tx_hash)}>
                            Watch it on the explorer
                          </ExplorerLink>
                        ) : null}
                    ...
                    {passport.status === "failed" ? (
                      <div className="space-y-2">
                        <p className="text-destructive">
                          {passport.failure_reason ?? "The last attempt failed."}
                        </p>
                        <Button
                          variant="ghost"
                          size="sm"
                    ...
                    {mintPreview ? (
                      <div className="space-y-1 rounded-lg border border-border p-3">
                        ...
                        <Field
                          label="Function"
                          value="mintPassport(to, listingId, metadataURI, metadataHash, termsHash)"
                        />
                        ...
                        <Field
                          label="Estimated gas"
                          value={
                            mintPreview.gasEstimate ??
                            `Unavailable — ${mintPreview.gasError ?? "estimate failed"}`
                          }
                        />
                        <Field
                          label="Estimated fee"
                          value={
                            mintPreview.feeEstimateEth ? `${mintPreview.feeEstimateEth} ETH` : "—"
                          }
                        />
                        <Field label="Your balance" value={`${mintPreview.balanceEth} ETH`} />
                        <Field label="Calldata" value={mintPreview.data} />
                        {mintPreview.insufficientFunds ? (
                          <Problems
                            items={[
                              "Your wallet does not hold enough ETH to pay for this transaction.",
                            ]}
                          />
                        ) : null}
                        {mintPreview.gasError && !mintPreview.insufficientFunds ? (
                          <Problems
                            items={[
                              `The node rejected a simulation of this call: ${mintPreview.gasError}`,
                            ]}
                          />
                        ) : null}
                      </div>
                    ...
                    {passport.status !== "submitted" ? (
                      <div className="flex flex-wrap gap-3">
                        {!mintPreview ? (
                          <Button
                            onClick={onPrepareMint}
                            disabled={busy !== null || !canSign || passport.status === "failed"}
                          >
                          ...
                          <>
                            <Button
                              onClick={onMint}
                              disabled={busy !== null || !canSign || mintPreview.insufficientFunds}
                            >
              ...
              <Step
                index={3}
                title="Companion Token (optional)"
                done={companion?.status === "confirmed"}
              >
                {passport?.status !== "confirmed" ? (
                  <p className="text-muted-foreground">
                    Available once the Item Passport is verified on-chain.
                  </p>
                ) : companion?.status === "confirmed" ? (
                  <div className="space-y-1">
                    <Field
                      label="Token"
                      value={`${companion.name} (${companion.symbol})`}
                      mono={false}
                    />
                    <Field label="Address" value={companion.token_address ?? ""} />
                    <Field
                      label="Fixed supply"
                      value={`${fromTokenUnits(String(companion.total_supply))} ${companion.symbol}`}
                      mono={false}
                    />
                    <Field
                      label="Your allocation"
                      value={`${fromTokenUnits(String(companion.creator_allocation))} ${companion.symbol}`}
                      mono={false}
                    />
                    ...
                    <div className="flex flex-wrap gap-4 pt-1">
                      <ExplorerLink
                        href={explorerTokenUrl(companion.chain_id, companion.token_address ?? "")}
                      >
                        Token on the explorer
                      </ExplorerLink>
                      {companion.tx_hash ? (
                        <ExplorerLink href={explorerTxUrl(companion.chain_id, companion.tx_hash)}>
                    ...
                    <p className="pt-1 text-muted-foreground">
                      One companion token per passport. This pairing is permanent and enforced by
                      the registry.
                    </p>
                  </div>
                ) : (
                  <>
                    {companion?.status === "submitted" ? (
                      <div className="space-y-2 rounded-lg border border-border bg-muted/30 p-3">
                        <p>
                          ...
                          <span className="font-mono text-xs">{companion.tx_hash}</span>
                        </p>
                        <div className="flex flex-wrap gap-3">
                          <Button
                            variant="outline"
                    ...
                    {companion?.status === "failed" ? (
                      <div className="space-y-2">
                        <p className="text-destructive">
                          {companion.failure_reason ?? "The last attempt failed."}
                        </p>
                        <Button
                          variant="ghost"
                          size="sm"
                      ...
                      <>
                        <div className="grid gap-3 sm:grid-cols-2">
                          ...
                          <div>
                            <Label htmlFor="token-supply">
                              Fixed total supply (whole tokens, immutable)
                            </Label>
                            <Input
                              id="token-supply"
                              inputMode="numeric"
                          ...
                          <div>
                            <Label htmlFor="token-allocation">
                              Your allocation (rest goes to the treasury)
                            </Label>
                            <Input
                              id="token-allocation"
                              inputMode="numeric"
                        ...
                        {tokenPreview ? (
                          <div className="space-y-1 rounded-lg border border-border p-3">
                            ...
                            <Field
                              label="Function"
                              value="createCompanionToken(passportTokenId, name, symbol, totalSupply, creatorAllocation)"
                            />
                            ...
                            <Field
                              label="passportTokenId"
                              value={tokenPreview.args.passportTokenId}
                            />
                            <Field
                              label="name / symbol"
                              value={`${tokenPreview.args.name} / ${tokenPreview.args.symbol}`}
                              mono={false}
                            />
                            <Field
                              label="totalSupply"
                              value={`${fromTokenUnits(tokenPreview.args.totalSupply)} (${tokenPreview.args.totalSupply} base units)`}
                            />
                            <Field
                              label="creatorAllocation"
                              value={`${fromTokenUnits(tokenPreview.args.creatorAllocation)} → ${shortAddress(tokenPreview.wallet)}`}
                            />
                            <Field
                              label="treasury"
                              value={`${fromTokenUnits(tokenPreview.args.treasuryAllocation)} → treasury`}
                            />
                            <Field
                              label="Estimated gas"
                              value={
                                tokenPreview.gasEstimate ??
                                `Unavailable — ${tokenPreview.gasError ?? "estimate failed"}`
                              }
                            />
                            <Field
                              label="Estimated fee"
                              value={
                                tokenPreview.feeEstimateEth
                                  ? `${tokenPreview.feeEstimateEth} ETH`
                                  : "—"
                              }
                            />
                            <Field label="Your balance" value={`${tokenPreview.balanceEth} ETH`} />
                            {tokenPreview.insufficientFunds ? (
                              <Problems
                                items={[
                                  "Your wallet does not hold enough ETH to pay for this transaction.",
                                ]}
                            ...
                            {tokenPreview.gasError && !tokenPreview.insufficientFunds ? (
                              <Problems
                                items={[
                                  `The node rejected a simulation of this call: ${tokenPreview.gasError}`,
                                ]}
                              />
                            ) : null}
                          </div>
                          ...
                          {!tokenPreview ? (
                            ...
                            <>
                              <Button
                                ...
                                disabled={
                                  busy !== null || !canSign || tokenPreview.insufficientFunds
                                }
                              >
              ...
              {!lpInfra?.available ? (
                ...
                <Step
                  index={4}
                  title="Uniswap v3 liquidity (optional, mainnet)"
                  done={liquidity?.status === "confirmed"}
                >
                  {companion?.status !== "confirmed" ? (
                    <p className="text-muted-foreground">
                      Available once the Companion Token is verified and paired.
                    </p>
                    ...
                    <div className="space-y-1">
                      ...
                      <Field
                        label="Fee tier"
                        value={`${liquidity.fee_tier / 10_000}%`}
                        mono={false}
                      />
                      <Field
                        label="Range"
                        value={`Full range (ticks ${liquidity.tick_lower} to ${liquidity.tick_upper})`}
                        mono={false}
                      />
                      <div className="flex flex-wrap gap-4 pt-1">
                        {liquidity.pool_address ? (
                          <ExplorerLink
                            href={explorerAddressUrl(liquidity.chain_id, liquidity.pool_address)}
                          >
                            Pool on the explorer
                          </ExplorerLink>
                        ) : null}
                        {liquidity.mint_tx_hash ? (
                          <ExplorerLink
                            href={explorerTxUrl(liquidity.chain_id, liquidity.mint_tx_hash)}
                          >
                            Position mint transaction
                          </ExplorerLink>
                        ) : null}
                        {liquidity.pool_tx_hash ? (
                          <ExplorerLink
                            href={explorerTxUrl(liquidity.chain_id, liquidity.pool_tx_hash)}
                          >
                      ...
                      {liquidity.lock_verified_at ? (
                          ...
                          <Field
                            ...
                            value={
                              liquidity.lock_permanent
                                ? "Never — this lock is permanent"
                                : new Date(liquidity.lock_unlock_at!).toUTCString()
                            }
                            mono={false}
                          />
                          <div className="flex flex-wrap gap-4 pt-1">
                            {liquidity.locker_address ? (
                              ...
                              >
                                Locker on the explorer
                              </ExplorerLink>
                            ) : null}
                            {liquidity.lock_tx_hash ? (
                              <ExplorerLink
                                href={explorerTxUrl(liquidity.chain_id, liquidity.lock_tx_hash)}
                              >
                                Lock transaction
                              </ExplorerLink>
                            ) : null}
                        ...
                        <div className="space-y-2 pt-2">
                          <p className="text-muted-foreground">
                            Liquidity is <strong>not locked</strong>. The position NFT is in your
                            wallet and you can remove the liquidity at any time. You can lock it in
                            the YARD SALE locker for at least 180 days, or permanently.
                          </p>
                    ...
                    <>
                      <ol className="flex flex-wrap gap-2 text-xs">
                        {(
                          ...
                          return (
                            <li
                              key={s}
                              className={`rounded-full border px-2.5 py-1 ${done ? "border-transparent bg-primary text-primary-foreground" : active ? "border-foreground font-semibold" : "border-border text-muted-foreground"}`}
                            >
                      ...
                      <div className="space-y-1">
                        <Field
                          label="Deposit"
                          value={`${fromTokenUnits(liquidity.token_amount)} ${companion.symbol} + ${fromTokenUnits(liquidity.eth_amount)} ETH`}
                          mono={false}
                        />
                        <Field
                          label="Slippage"
                          value={`${liquidity.slippage_bps / 100}%`}
                          mono={false}
                        />
                        ...
                        {liquidity.failure_reason ? (
                          <p className="text-destructive">{liquidity.failure_reason}</p>
                        ) : null}
                      </div>
                      <div className="flex flex-wrap gap-3">
                        <Button onClick={onContinueLiquidity} disabled={busy !== null || !canSign}>
                          ...
                          {liquidity.status === "failed"
                            ...
                            : "Continue: sign the next step"}
                        </Button>
                        <Button
                          variant="outline"
                          onClick={onVerifyLiquidity}
                          disabled={busy !== null}
                        >
                    ...
                    <>
                      <p className="text-muted-foreground">
                        Seeds a full-range 0.3% {companion.symbol}/WETH pool through the official
                        Uniswap v3 deployment on Robinhood Chain. Five wallet signatures at most:
                        wrap, two exact approvals, pool creation and the position mint.
                      </p>
                      <div className="grid gap-3 sm:grid-cols-3">
                        <div>
                          ...
                          <Input
                            ...
                            placeholder={fromTokenUnits(String(companion.creator_allocation))}
                            disabled={Boolean(lpPreview)}
                          />
                        </div>
                        <div>
                      ...
                      {lpPreview ? (
                        <div className="space-y-1 rounded-lg border border-border p-3">
                          ...
                          <Field
                            label="Opening price"
                            value={`${lpPreview.plan.priceEthPerToken} ETH per ${companion.symbol} (${lpPreview.plan.tokensPerEth} ${companion.symbol} per ETH)`}
                            mono={false}
                          />
                          <Field
                            label="Fully diluted valuation"
                            value={`${lpPreview.plan.fdvEth} ETH at the opening price`}
                            mono={false}
                          />
                          <Field
                            label="Pair"
                            value={`${lpPreview.plan.token0} / ${lpPreview.plan.token1}`}
                          />
                          <Field
                            label="Fee tier / range"
                            value={`${lpPreview.feeTier / 10_000}% · full range (${lpPreview.plan.tickLower} to ${lpPreview.plan.tickUpper})`}
                            mono={false}
                          />
                          <Field label="sqrtPriceX96" value={lpPreview.plan.sqrtPriceX96} />
                          <Field
                            label="Minimum amounts"
                            value={`${lpPreview.plan.amount0Min} / ${lpPreview.plan.amount1Min} base units (${lpPreview.plan.slippageBps / 100}% slippage)`}
                          />
                          <Field
                            label="Balances"
                            value={`${lpPreview.balances.eth} ETH · ${lpPreview.balances.weth} WETH · ${fromTokenUnits(lpPreview.balances.tokenWei)} ${companion.symbol}`}
                            mono={false}
                          />
                          <Field
                            label="Steps"
                            value={
                              <ul className="list-disc pl-4">
                                {lpPreview.steps.map((s) => (
                                  <li
                                    key={s.step}
                                    className={s.needed ? "" : "text-muted-foreground line-through"}
                                  >
                                    {s.label}
                                    {s.gasEstimate ? ` (~${s.gasEstimate} gas)` : ""}
                                  </li>
                                ))}
                              </ul>
                            }
                          ...
                          <Field
                            ...
                            value={`~${lpPreview.feeEstimateEth} ETH in gas`}
                            mono={false}
                          />
                      ...
                      <fieldset className="space-y-2 rounded-lg border border-border p-3">
                        ...
                        {LIQUIDITY_RISK_STATEMENTS.map((text, i) => (
                          <label key={text} className="flex items-start gap-3">
                            <Checkbox
                              checked={lpRisks[i] === true}
                              onCheckedChange={(value) =>
                                setLpRisks((r) => r.map((v, j) => (j === i ? value === true : v)))
                              }
                              aria-label={text}
                            />
                      ...
                      <div className="flex flex-wrap gap-3">
                        {!lpPreview ? (
                          <Button
                            onClick={onPreviewLiquidity}
                            disabled={busy !== null || !canSign || !lpTokenAmount || !lpEthAmount}
                          >
                          ...
                          <>
                            <Button
                              onClick={onStartLiquidity}
                              disabled={
                                busy !== null ||
                                !canSign ||
                                !lpRisks.every(Boolean) ||
                                lpPreview.problems.length > 0
                              }
                            >
